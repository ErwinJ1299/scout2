...
interface ProcessedFeatures {
  ...
  calories_mean: number;
  calories_std: number;
  age: number;
  gender_encoded: number;
  days_with_data: number;
  trend_heart_rate: number;
  trend_steps: number;
  trend_sleep: number;
}
...
function computeFeatures(
  ...
  const uniqueDays = new Set(
    metrics.map(m => m.timestamp.toDate().toDateString())
  );
 ...
 */
function calculateRiskScore(features: ProcessedFeatures): number {
  let riskScore = 0.3; // Base risk
  
  // Heart rate risk factors
  if (features.heart_rate_mean > 80) riskScore += 0.15;
  else if (features.heart_rate_mean > 75) riskScore += 0.08;
  else if (features.heart_rate_mean < 60) riskScore -= 0.05;
  
  // Activity risk factors
  if (features.steps_mean < 5000) riskScore += 0.20;
  else if (features.steps_mean < 7000) riskScore += 0.10;
  else if (features.steps_mean > 10000) riskScore -= 0.08;
  
  // Sleep risk factors
  if (features.sleep_hours_mean < 6) riskScore += 0.18;
  else if (features.sleep_hours_mean < 7) riskScore += 0.10;
  else if (features.sleep_hours_mean > 8) riskScore -= 0.05;
  
  // Calories risk factor
  if (features.calories_mean > 2800) riskScore += 0.10;
  else if (features.calories_mean < 1800) riskScore += 0.08;
  
  // Age factor
  if (features.age > 60) riskScore += 0.10;
  else if (features.age > 50) riskScore += 0.05;
  
  // Trend factors (worsening trends increase risk)
  if (features.trend_heart_rate > 1) riskScore += 0.08;
  if (features.trend_steps < -100) riskScore += 0.08;
  if (features.trend_sleep < -0.2) riskScore += 0.08;
...
export const getHealthHistory = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    metricsSnapshot.forEach(doc => {
      ...
      if (data.metricType === 'heart_rate') dailyData[date].heart_rate.push(data.value);
      if (data.metricType === 'steps') dailyData[date].steps.push(data.value);
      if (data.metricType === 'sleep') dailyData[date].sleep_hours.push(data.value);
      if (data.metricType === 'calories') dailyData[date].calories.push(data.value);
    });

    // Calculate daily averages
    const history = Object.values(dailyData).map((day: any) => ({
      date: day.date,
      ...
      sleep_hours: day.sleep_hours.length > 0
        ? day.sleep_hours.reduce((a: number, b: number) => a + b, 0) / day.sleep_hours.length
        : 0,
      calories: day.calories.length > 0
        ? day.calories.reduce((a: number, b: number) => a + b, 0) / day.calories.length
        : 0
    }));
 ...
 */
export const predictHealthRisk = functions
